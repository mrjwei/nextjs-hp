...
export function generateMetadata({ params }) {
  ...
  if (writing) {
    ...
    const pathname = `/posts/${writing.slug}`
...
export default async function SlugOrCollectionPage({ params, searchParams }) {
  ...
  if (writing?.metadata.series) {
    permanentRedirect(getWritingHref(writing))
  }
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                url: `${baseUrl}/posts/${writing.slug}`,
                author: {
                  "@type": "Person",
  ...
  const sidebarSeries = getAllSortedWritingCollections().map((s) => ({
    slug: s.slug,
    title: s.title,
  ...
  const activeSeries = getAllSortedWritingSeries().find((s) => s.slug === slugOrCollection)
